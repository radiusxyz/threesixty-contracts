...
describe("TexFactory", () => {
  ...
  async function createPair(tokens: [string, string]) {
    const bytecode = `0x${TexPair.evm.bytecode.object}`;
    ...
    await expect(factory.createPair(...tokens)).to.be.reverted; // Tex: PAIR_EXISTS
    await expect(factory.createPair(...tokens.slice().reverse())).to.be
      .reverted; // Tex: PAIR_EXISTS
  ...
  it("setFeeTo", async () => {
    ...
    ).to.be.revertedWith("Tex: FORBIDDEN");
