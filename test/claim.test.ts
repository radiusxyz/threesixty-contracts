  ...
  it("three-sixty test", async function () {
    ...
    for (let i = 1; i <= userCount; i++) {
      ...
      if (i == 2) {
        ...
        const signatureHex = await operator._signTypedData(
          typedData.domain,
    ...
    const userRouterContract = threesixtyRouter02.connect(clients[1].signer);

    console.log("txOrderMsg", txOrderMsg);
    console.log();
    ...
    const client1Balance = await middleTokenContract.balanceOf(
      clients[1].address
    );
    const client2Balance = await middleTokenContract.balanceOf(
      clients[2].address
    );
    const vaultBalance = await middleTokenContract.balanceOf(vault.address);

    console.log();
    console.log("client1Balance", client1Balance);
    console.log("client2Balance", client2Balance);
