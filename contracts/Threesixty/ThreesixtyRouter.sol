...
contract ThreesixtyRouter is IThreesixtyRouter {
  ...
  modifier ensure(uint256 deadline) {
    require(deadline >= block.timestamp, "ThreesixtyRouter: EXPIRED");
    _;
  }

  event RunBacker(bool);
  ...
  function setFeeTo(address _feeTo) external {
    require(msg.sender == feeToSetter, "Threesixty: FORBIDDEN");
    feeTo = _feeTo;
  }

  function setFeeToSetter(address _feeToSetter) external {
    require(msg.sender == feeToSetter || msg.sender == owner, "Threesixty: FORBIDDEN");
    feeToSetter = _feeToSetter;
  }

  function setOperator(address _operator) external {
    require(msg.sender == operatorSetter, "Threesixty: FORBIDDEN");
    operator = _operator;
  }

  function setOperatorSetter(address _operatorSetter) external {
    require(msg.sender == operatorSetter || msg.sender == owner, "Threesixty: FORBIDDEN");
    operatorSetter = _operatorSetter;
  }
    ...
    if (reserveA == 0 && reserveB == 0) {
      ...
      if (amountBOptimal <= amountBDesired) {
        require(
          ...
          "ThreesixtyRouter: INSUFFICIENT_B_AMOUNT"
        );
        ...
        require(
          ...
          "ThreesixtyRouter: INSUFFICIENT_A_AMOUNT"
        );
    ...
    returns (uint256 amountA, uint256 amountB)
  {
    ...
    (amountA, amountB) = tokenA == token0
      ? (amount0, amount1)
      : (amount1, amount0);
    require(amountA >= amountAMin, "ThreesixtyRouter: INSUFFICIENT_A_AMOUNT");
    require(amountB >= amountBMin, "ThreesixtyRouter: INSUFFICIENT_B_AMOUNT");
  }
  ...
  function claim(
    ...
    require(
      ...
      "ThreesixtyRouter: TX hash and proof hash are available!!"
    );

    require(
      proofHash != resultHash,
      "ThreesixtyRouter: Signature is not valid"
    );
    ...
    require(
      ...
      "ThreesixtyRouter: Claim is not accepted. Check signature"
    );
  }
    ...
    returns (uint256[] memory amounts)
  {
    ...
    require(
      ...
      "ThreesixtyRouter: INSUFFICIENT_OUTPUT_AMOUNT"
    );
    ...
    if (amounts[0] < amounts[3]) {
      IBacker(backer).swap(amounts[0], backerPath, deadline);
      emit RunBacker(true);
    } else {
      emit RunBacker(false);
    }
  }
    ...
    returns (uint256[] memory amounts)
  {
    ...
    require(amounts[0] <= amountInMax, "ThreesixtyRouter: EXCESSIVE_INPUT_AMOUNT");
    TransferHelper.safeTransferFrom(
      path[0],
      origin,
      UniswapV2Library.pairFor(factory, path[0], path[1]),
  ...
  function swapExactETHForTokens(
    ...
    returns (uint256[] memory amounts)
  {
    require(path[0] == WETH, "ThreesixtyRouter: INVALID_PATH");
  ...
  function swapTokensForExactETH(
    ...
    returns (uint256[] memory amounts)
  {
    require(path[path.length - 1] == WETH, "ThreesixtyRouter: INVALID_PATH");
    amounts = UniswapV2Library.getAmountsIn(factory, amountOut, path);
    require(amounts[0] <= amountInMax, "ThreesixtyRouter: EXCESSIVE_INPUT_AMOUNT");
    TransferHelper.safeTransferFrom(
      path[0],
    ...
    returns (uint256[] memory amounts)
  {
    require(path[0] == WETH, "ThreesixtyRouter: INVALID_PATH");
    amounts = UniswapV2Library.getAmountsIn(factory, amountOut, path);
    require(amounts[0] <= msg.value, "ThreesixtyRouter: EXCESSIVE_INPUT_AMOUNT");
    IWETH(WETH).deposit{ value: amounts[0] }();
    assert(
      IWETH(WETH).transfer(
        UniswapV2Library.pairFor(factory, path[0], path[1]),
  ...
  function swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    require(amountOut >= amountOutMin, "ThreesixtyRouter: INSUFFICIENT_OUTPUT_AMOUNT");
