...
import "./SafeMath.sol";
import "./TexLibrary.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library TexLiquidityMathLibrary {
  ...
  function getReservesAfterArbitrage(
    ...
    (reserveA, reserveB) = TexLibrary.getReserves(factory, tokenA, tokenB);

    require(
      reserveA > 0 && reserveB > 0,
      "TexArbitrageLibrary: ZERO_PAIR_RESERVES"
    );
    ...
    if (aToB) {
      uint256 amountOut = TexLibrary.getAmountOut(amountIn, reserveA, reserveB);
      reserveA += amountIn;
      reserveB -= amountOut;
    } else {
      uint256 amountOut = TexLibrary.getAmountOut(amountIn, reserveB, reserveA);
      reserveB += amountIn;
      reserveA -= amountOut;
    }
  }
  ...
  function getLiquidityValue(
    ...
    (uint256 reservesA, uint256 reservesB) = TexLibrary.getReserves(
      factory,
      tokenA,
      tokenB
    );
    ITexPair pair = ITexPair(TexLibrary.pairFor(factory, tokenA, tokenB));
    bool feeOn = ITexFactory(factory).feeTo() != address(0);
